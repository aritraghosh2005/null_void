...
export default function Board() {
  ...
  const [isMounted, setIsMounted] = useState(false); // [FIX] Hydration state

  // Panning State
  const isPanning = useRef(false);
  const lastMouse = useRef({ x: 0, y: 0 });

  // [FIX] Wait for mount to avoid Hydration Error
  useEffect(() => {
    setIsMounted(true);
  }, []);
          ...
          if (file) {
            ...
            reader.onload = (event) => {
              ...
              img.onload = () => {
                ...
                // Spawn center of SCREEN, converted to WORLD coordinates
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    const newScale = Math.max(0.1, Math.min(view.scale * (1 + factor), 5)); // Cap 0.1x to 5x

    // Zoom towards the mouse pointer math
    // 1. Mouse before zoom (World)
    const wx = (e.clientX - view.x) / view.scale;
    const wy = (e.clientY - view.y) / view.scale;

    // 2. Calculate new View Offset
  ...
  // --- 3. PAN LOGIC (Drag Background) ---
  const handleMouseDown = (e: React.MouseEvent) => {
    // Only pan if clicking direct background (not a pin)
  ...
  const handleDoubleClick = (e: React.MouseEvent) => {
    // Spawn at clicked World coordinates
      ...
      */}
      <div 
        style={{
          ...
          transformOrigin: '0 0', // Important for our custom zoom math
          willChange: 'transform',
        }}
        className="absolute inset-0 w-full h-full"
      >
        {/* Parallax Grid Background (Now moves with the world!) */}
        <div 
          className="absolute inset-[-500%] w-[1000%] h-[1000%] pointer-events-none opacity-20"
          style={{
        ...
        {/* Render Pins (Only after mount to fix hydration) */}
        {isMounted && pins.map((pin) => (
          <PinCard key={pin.id} pin={pin} />
        ))}
      </div>

      {/* UI Overlay (Stays fixed on screen) */}
      <div className="absolute top-12 left-12 pointer-events-none z-[100] select-none">
        ...
        <div className="flex items-center gap-3 mt-2 pl-1 opacity-70">
          ...
          <p className="font-mono text-xs text-emerald-400/80 tracking-widest uppercase">
            {/* Show Zoom Level */}
            Scale: {Math.round(view.scale * 100)}% // Pos: {Math.round(view.x)},{Math.round(view.y)}
          </p>
        </div>
      </div>

      {/* Liquid Cursor (Stays fixed on screen) */}
      <MouseTrail />
    </main>
  );
}